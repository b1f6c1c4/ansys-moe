...
\begin{figure}[p]
  \centering%
  %\subcaptionbox{建模过程中用到的正方形线圈的轮廓\label{fig:example-curve}}
  %  {\includegraphics{./data/dist/raw-pDimension-pThickness.pdf}}\par
  \subcaptionbox{最终完成的几何模型\label{fig:example-overall}}
    {\includegraphics{./data/dist/raw-dCoilOuter-dShieldThickness.pdf}}\par
  \subcaptionbox{最终完成的几何模型\label{fig:example-overall}}
    {\includegraphics{./data/dist/raw-dCoilOuter-dShieldExtra.pdf}}
  \caption[几何模型的搭建]{使用Ansys Maxwell 3D对磁谐振部件的几何结构进行建模。}
\end{figure}
