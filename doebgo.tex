...
为了方便起见，本文采用R语言\verb"GPfit"包\cite{macdonald2015}从实验数据中估计高斯过程的三类参数：
...
实际上，虽然我们可以获得$Y(x_\star), Y(x_{\ast 1}),\ldots,Y(x_{\ast p})$的联合后验分布$\mathcal{D}^\dag$，但数学期望依然没有闭式解。
...
\begin{align}\label{equ:bgo-mc}
  \alpha^k &= \max\!\left\{\tau - \min\{y_\star^k, y_{\ast 1}^k,\ldots,y_{\ast p}^k\}\} \\
...
再结合文献~\inlincite{deutsch2012}从实验设计角度对各种LHS方法的扩展的讨论，本文最终采用LHSMDU方法对贝叶斯优化进行初始化。
