...
输入弧不得跨层；跨层的输出弧从某一$k$次嵌套（每含在一层内算作一次嵌套，不含在任何层内为0层嵌套，下同）的跳变开始，
...
L-Petri网的执行即为传统Petri网的执行，不过对于跨层弧翻译而来输出弧的处理略有不同：
...
因此本小节将二者融合，引入FL-Petri网的概念，并用它来建模传统Petri网和UML活动图都无法建模的系统，
